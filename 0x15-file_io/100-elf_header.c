 ...
 *
 * Description: if the file is not an ELF file - exist code 98
 */
void check_elf(unsigned char *e_ident)
{
	int index;
 ...
 * @e_ident: apointer to an array containg the ELF version
 */
void print_osabi(unsigned char *e_ident)
{
	printf("  OS/ABI:                         ");
 ...
 * @e_type: the ELF type
 * @e_ident: a pointer to an array cotaining the ELF class
 */
 ...
 * Return: 0 on Success
 *
 * Description: if the file if not an ELF file or
...
int main(int __attribute__((__unused__)) argc, char *argv[])
{
	...
	if (o == -1)
	{
		printf(STDERR_FILENO, "Error: Can't read file %s\n", argv[1]);
		exit(98);
	}
